...
class map_avl
{
  protected:
    ...
    class node {
      ...
      protected:
        ...
        int get_height(node *n) const { // ��� OO ?
  ...
  public:
    ...
    //-------------- For student.h --------------
    std::vector<size_t> balance_values(iterator it) const;
    void my_recur(node* node, iterator it, std::vector<size_t> &val) const;
    void my_recur_2(node* node, iterator it, std::vector<size_t> &val) const;
};
